...
# Evaluation
parser.add_argument("--eval-every", type=int, default=250, help="evaluate val and train bpb every N steps (-1 = disable)")
parser.add_argument("--eval-tokens", type=int, default=80*524288, help="number of tokens to evaluate val and train loss on")
...
if not resuming:
    ...
    min_train_bpb = loop_state["min_train_bpb"]
...
while True:
    ...
    if args.eval_every > 0 and (last_step or step % args.eval_every == 0):
        ...
        train_loader_logging = build_train_loader()
        eval_steps = args.eval_tokens // (args.device_batch_size * args.max_seq_len * ddp_world_size)
        with disable_fp8(model):
            val_bpb = evaluate_bpb(model, val_loader, eval_steps, token_bytes)
            train_bpb = evaluate_bpb(model, train_loader_logging, eval_steps, token_bytes)
        print0(f"Step {step:05d} | Validation bpb: {val_bpb:.6f} | Train bpb: {train_bpb}")
        if val_bpb < min_val_bpb:
            min_val_bpb = val_bpb
        if train_bpb < min_train_bpb:
            min_train_bpb = train_bpb
...
if val_bpb is not None:
    ...
    print0(f"Minimum train bpb: {min_train_bpb:.6f}")
